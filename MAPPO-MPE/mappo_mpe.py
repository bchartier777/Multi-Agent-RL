...
from network_v2 import Actor_RNN, Critic_RNN, Actor_RNN_v2, Critic_RNN_v2, Actor_MLP, Critic_MLP, Actor_MLP_v2, Critic_MLP_v2
import numpy as np
...
class MAPPO_MPE:
    def __init__(self, args):
        ...
        if self.use_rnn:
            print("------use rnn------")
            self.actor = Actor_RNN_v2(args, self.actor_input_dim) # Actor_RNN_v2
            self.critic = Critic_RNN_v2(args, self.critic_input_dim)
        else:
        ...
        if self.set_adam_eps:
            print("------set adam eps------")
            self.ac_optimizer = torch.optim.AdamW(self.ac_parameters, lr=self.lr, eps=1e-5)
        else:
            self.ac_optimizer = torch.optim.AdamW(self.ac_parameters, lr=self.lr)

    # def choose_action(self, obs_n, evaluate):
    def choose_action(self, args, obs_n, evaluate, noise_std = 0.05):
        with torch.no_grad():
            actor_inputs = []
            # This is the original conversion, which generates a warning.  Replaced with the following.
            # obs_n = torch.tensor(obs_n, dtype=torch.float32)  # obs_n.shape=(N，obs_dim)
            obs_n_arr = np.stack(obs_n)
            obs_n = torch.from_numpy(obs_n_arr)
            obs_n = obs_n.type(torch.float32)
            ...
            # If adding Gaussian noise to action, convert to prob dist to satisfy Simplex constraint
            # Also only add noise when training
            if args.add_noise_action == True and evaluate == False:
                noise = torch.normal(mean=0, std=0.05, size=(3,5))
                prob = prob + noise
